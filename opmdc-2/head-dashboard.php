<<<<<<< HEAD:opmdc-2/head-dashboard.php
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        async function loadRequestsAndPopulate() {
            ...
            try {
                ...
                // fallback: include localStorage requests if server not available
                console.warn('Could not load server requests, falling back to localStorage:', err);
                const local = JSON.parse(localStorage.getItem('opmdcRequests')) || [];
                // normalize local requests to server shape
                requests = local.map(r => ({ id: r.id, barangay: r.barangay, request_type: r.requestType, created_at: r.date || new Date().toISOString(), status: r.status || 'Pending', history: r.history || [] }));
            }
            ...
            if (subBody) {
                ...
                if (requests.length === 0) {
                    ...
                    requests.slice(0, 50).forEach(r => {
                        const tr = document.createElement('tr');
                        tr.className = 'border-b hover:bg-gray-50';
                        tr.innerHTML = `<td class="py-3 px-4 font-medium">${escapeHtml(r.request_type || r.title || 'Request')}</td><td class="py-3 px-4">${escapeHtml(r.barangay || r.submitter || '')}</td><td class="py-3 px-4">${new Date(r.created_at).toLocaleDateString()}</td><td class="py-3 px-4">${getStatusLabel(r.status || '')}</td><td class="py-3 px-4"><a class="text-blue-500 hover:underline" href="list_requests.php?id=${encodeURIComponent(r.id)}">View</a></td>`;
                        subBody.appendChild(tr);
                    });
                }
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    headMarkAllRead && headMarkAllRead.addEventListener('click', async () => { const res = await fetch('notifications.php?role=' + encodeURIComponent('OPMDC Head')); const data = await res.json(); for (const n of data.notifications || []) { await fetch('notifications_mark_read.php?id=' + encodeURIComponent(n.id), { method: 'POST' }); } fetchHeadNotifs(); });
        ...
        // Realtime notifications via SSE for Head role
        (function startHeadSSE(){
            try {
                const last = parseInt(localStorage.getItem('opmdcHeadLastNotifId')||'0',10) || 0;
                const es = new EventSource('notifications_stream.php?last_id=' + last);
                es.addEventListener('notification', (e) => {
                    try {
                        const data = JSON.parse(e.data);
                        const id = parseInt(data.id||0,10);
                        if (id) localStorage.setItem('opmdcHeadLastNotifId', String(id));
                        headBadge && headBadge.classList.remove('hidden');
                        if (headDropdown && !headDropdown.classList.contains('hidden')) {
                            fetchHeadNotifs();
                        }
                    } catch(err) {}
                });
                es.addEventListener('error', () => { /* browser auto-reconnects */ });
            } catch (err) { /* SSE unsupported */ }
        })();
...
>>>>>>> 996c8d0d135fff7224812be0b39c025218bf85f0:opmdc-2/head-dashboard.html
