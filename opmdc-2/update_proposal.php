...
// If a staff member forwards the proposal for approval, notify the Head
try {
    if (isset($user['role']) && $user['role'] === 'OPMDC Staff' && strcasecmp($status, 'For Approval') === 0) {
        $hTitle = "Proposal forwarded for approval";
        $hBody = sprintf("Proposal '%s' (ID #%d) has been forwarded for approval by %s", $proposal['title'] ?? ('#' . $id), $id, $user['role'] ?? 'OPMDC Staff');
