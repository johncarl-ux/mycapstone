...
// 3) Proposals by barangay (top 10) — apply optional range
try {
    ...
    if ($res) {
        while ($r = $res->fetch_assoc()) {
            $out['proposals_by_barangay'][] = ['barangay' => $r['barangay'], 'count' => intval($r['c'])];
        }
        $res->free();
    }
} catch (Exception $e) { }
...
try {
    ...
    $res = $mysqli->query("SELECT barangay, COUNT(*) AS c FROM project_proposals WHERE 1=1 " . $rangeConditions . " GROUP BY barangay ORDER BY c DESC LIMIT 10");
    $bars = [];
    ...
    if (count($bars) > 0) {
        ...
        $sql = "SELECT DATE_FORMAT(created_at, '%Y-%m') AS ym, barangay, COUNT(*) AS c FROM project_proposals WHERE 1=1 " . $rangeConditions . " AND barangay IN (".$inList.") GROUP BY ym, barangay ORDER BY ym ASC";
        $res = $mysqli->query($sql);
        $cells = [];
        if ($res) {
...
try {
    ...
    // Top barangay
    if (count($out['proposals_by_barangay']) > 0) {
        $top = $out['proposals_by_barangay'][0];
    $totalProposals = intval($out['counts']['total_proposals'] ?? 0);
    ...
    if ($pct >= 30) $sev = 'high'; elseif ($pct >= 18) $sev = 'medium';
        $insights[] = ['message' => "Highest proposal volume: {$top['barangay']} ({$top['count']} proposals, {$pct}% of total) — consider workload balancing.", 'severity' => $sev, 'code' => 'TOP_BARANGAY'];
    }
    ...
    // Removed OLD_PENDING (requests deprecated)
